...
from transformers import ViTForImageClassification, AutoImageProcessor ,ResNetForImageClassification, ViTImageProcessor
...
def main(cfg: DictConfig):
    ...
    model = ResNetForImageClassification.from_pretrained(cfg.modeling.teacher.checkpoint_path) if cfg.modeling.type == "resnet" else ViTForImageClassification.from_pretrained(cfg.modeling.teacher.checkpoint_path)
    ...
    if cfg.modeling.student.finetune:
        load_checkpoint = os.path.join(f"checkpoints/{cfg.modeling.student.checkpoint_name}.pt")
