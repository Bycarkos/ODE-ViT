...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        ## Slightly MOdified
        attn = F.softmax(score, -1) #* (query - key) ## !! Bsed on The Emergence of Clusters in self attention dynamics
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int = 512, num_heads: int = 8):
        ...
        self.out_proj = nn.Linear(self.num_heads * self.d_head, d_model)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        self.token_len = max_len
        self._num_tokens = d_model
    ...
    def get_sinusoid_encoding(self):
        ...
        def get_position_angle_vec(i):
            return [i / np.power(10000, 2 * (j // 2) / self.token_len) for j in range(self.token_len)]

        sinusoid_table = np.array([get_position_angle_vec(i) for i in range(self._num_tokens)])
        sinusoid_table[:, 0::2] = np.sin(sinusoid_table[:, 0::2])
        sinusoid_table[:, 1::2] = np.cos(sinusoid_table[:, 1::2])

        self.pe = torch.FloatTensor(sinusoid_table).unsqueeze(0).to(device)
