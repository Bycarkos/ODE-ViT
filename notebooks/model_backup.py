...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor, learn_ivp: bool = False) -> torch.Tensor:
        ...
        x += self.pos_embed[:, : self.num_patches, :].to(x.device)
...
class ViTNeuralODE(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            ...
            register_tokens=register_tokens,  ## AD-HOC
        )
    ...
    def compute_upper_bound_by_second_derivative(self, R, L):
        Wq, Wk, Wv = self.odefunc.block.attn.mha.in_proj_weight.reshape(3, 768, 768)
    ...
    def forward(
        ...
        if output_attention:
            ...
            out["jasmin_loss"] = self.jasmin_loss(
                self.odefunc.block.attentions, k=jasmin_k, reduction="mean"
            )
