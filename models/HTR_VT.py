...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, mask_ratio=0.4, max_span_length=8, use_masking=True):
        # embed patches
        pixel_values = self.layer_norm(pixel_values)
        pixel_values = self.patch_embed(pixel_values)
        b, c, w, h = pixel_values.shape
        pixel_values = pixel_values.view(b, c, -1).permute(0, 2, 1)
        # masking: length -> length * mask_ratio
        if use_masking:
            pixel_values = self.random_masking(pixel_values, mask_ratio, max_span_length)
        
        cls_tokens = self.cls_token.repeat(b, 1, 1)
        pixel_values = pixel_values + self.pos_embed

        pixel_values = torch.cat((cls_tokens, pixel_values), dim=1)
        # apply Transformer blocks
        for blk in self.blocks:
            pixel_values = blk(pixel_values)

        pixel_values = self.norm(pixel_values)
        # To CTC Loss
        preds = self.head(pixel_values)
        preds = self.layer_norm(preds)
