...
class ViT_ODEFunc(nn.Module):
    ...
    def __init__(
        ...
        self.block = ParallelAttentionMLP(
            dim, num_heads, mlp_ratio, attn_drop, proj_drop, mlp_drop
        )
        self.conditional_time_embedding = nn.Linear(
            dim * 2, dim
        )  # MLP(dim=dim, hidden_dim=dim, drop=0.0)
...
class ViTNeuralODE(nn.Module):
    ...
    def forward(
        ...
        teacher_states: Optional[
            torch.Tensor
        ] = None,  # [L, B, N, D] (optional: baseline ViT block outputs)
