...
class SegmentEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.time_var_encoding =  MLP(f_in=segment_length*input_dim, f_out=hidden_dim, hidden_layers=3, hidden_dim=hidden_dim, activation="relu")

        self.observation_encoder = MLP(f_in=2*hidden_dim, f_out=K_dim, hidden_layers=3, hidden_dim=hidden_dim, activation="relu")
    ...
    def forward(self, x_enc: torch.Tensor):
        ...
        memories = self.layer_norm(memories)
        ...
        observation = self.observation_encoder(memories)
...
class SegmentDecoder(nn.Module):
    def __init__(self, encoder_input_dim: int, hidden_dim:int,  K_dim: int=8):
        ...
        self.decoder = MLP(f_in=K_dim, f_out=encoder_input_dim, hidden_layers=4, hidden_dim=hidden_dim, activation="relu")
